...
# --------------- CORS 中间件 (已硬编码为允许所有来源) ---------------
# !!!! 注意: 这里的CORS策略被硬编码为允许所有来源 ("*") !!!!
# 这提供了最大的兼容性，但也可能带来安全风险，请根据实际部署环境评估。
app.add_middleware(
    CORSMiddleware,
...
def open_browser():
    ...
    try:
        # 作者代码使用的是7860，这里保持一致
        url_to_open = f"http://127.0.0.1:7860{dashboard_path}"
        webbrowser.open(url_to_open)
        log('info', f"已发送打开浏览器指令: {url_to_open}")
